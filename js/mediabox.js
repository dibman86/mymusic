(function(root, factory) {
	...
	window.isplayerpaused = false;
	window.isMinimised = false;
	window.isFullScreen = false;
	let wrapper = null;
	let cell = null;
	
	const MediaBox = function(element, params) {
		let default_params = {
				enablejsapi: '1',
				controls: '0',
				showinfo: '0',
		...
		this.sceneenglobe = this.root.querySelector('.scene__englobe');
		...
		this.timererror,this.intervalId,this.player,this.currenttime,this.stylescene,this.keydownExecute,this.keyupExecute,this.toggleMiniLecteure;
	...
	MediaBox.prototype = {
		run: function() {
			...
			function playflicker() {
				...
				window.requestAnimationFrame(function(time) {
					marquee.className = "marquee flicker";
				});
			};

			const openLightbox = function(el) {
				if (!el.classList.contains('cell-center') || el.classList.contains('scrubbing') || document.querySelector('.not_result') !== null || (window.mediaboxopen && !window.isMinimised)) return;
		...
		events: function(service) {
			...
			const mediaboxbgoverlay = wrapper.querySelector('.mediabox-bg-overlay');
			const content = wrapper.querySelector('.mediabox-content');
			const mediaboxclose = content.querySelector('.mediabox-close');
			const lecteurMini = content.querySelector('.mini-lecteur');
			const setting = content.querySelector('.video-setting');
			const checkbox = content.querySelectorAll('[type="checkbox"][name="options"]');
			const videoContainer = document.querySelector(".video-container");
			const bigplayPauseBtn = videoContainer.querySelector(".ytplayer-cache");
			const skipContainer = bigplayPauseBtn.querySelector(".skip-container");
			const playPausebtncenter = skipContainer.querySelector(".big-playpause");
			const revenir = skipContainer.querySelector(".revenir");
			const avancer = videoContainer.querySelector(".avancer");
			const videoContrels = videoContainer.querySelector(".video-controls-container");
			const playPauseBtn = videoContainer.querySelector(".play-pause-btn");
			const nextvideo = videoContainer.querySelector('.next-btn');
			const previousvideo = videoContainer.querySelector('.previous-btn');
			...
			const thumbIndicator = videoContainer.querySelector(".thumb-indicator");
			const currentTimeSlider = timelineContainer.querySelector(".current-time-slider");
			let player,intervalId,wasPaused;
			let playerready = false;
			let sautvolume = 5;
			let timercursor,timeclickout,startY;
			const distance = 100;
			...
			function wheelvolume(e) {
				if (playerready) {
					...
					if (delta) delta < 0 ? volumeChange(-1) : volumeChange(1);
				}
			};
			...
			function addAttributeCheched(el,val) {
				...
				let ariatitle = label.getAttribute("aria-label");
				if(val === '1'){
			...
			(function initcheckbox() {
				let checkcookie = checkCookie('valuecheckbox');
			...
			const playTofs = function() {
				...
				timer = setTimeout(function() {
					...
					if(window.isMinimised){
						el = document.querySelector('a.active_mini');
						elclass = 'mini';
					}else{
					...
					if(!el) return;
					let tab = this.initElement(el,elclass);
			...
			const gestionError = function(event) {
				if (playerready) {
					...
					if(content.classList.contains('open-setting') || isScrubbing || isCloseHover) {
						...
						checkbox[1].checked ? triggerEvent(nextvideo, 'click') : this.close();
						
					}
				}
			}.bind(this);
			
			const onPlayerError = function() {
				if(this.timererror) clearTimeout(this.timererror);
				videoContainer.classList.add("playererror");
				videoContainer.classList.remove("playerstop");
				videoContainer.classList.remove("paused");
				removeNotes("mediabox-content");
			...
			const onPlayerReady = function(event) {
				window.mediaboxPlayer = this.player = player;
				event.target.setVolume(window.globalvolume);
				const duration = event.target.getDuration();
				(parseFloat(this.currenttime) <= 0 || parseFloat(this.currenttime) >= (duration - 5)) ? this.currenttime = '0' : this.currenttime -= 0.5;
				totalTimeElem.textContent = formatDuration(duration);
				window.globalmute ? changeIconeMute()  : event.target.setVolume(window.globalvolume);
				toogleIconVolume();
				...
				resultsplit = Splitting({
					target: ".grid",
					by: "cells",
					columns: 20 * Math.round(wrapper.offsetWidth/wrapper.offsetHeight),
					rows: 20
				})
				let holdTimeout = null;
				const endHolding = function(e){
					e.preventDefault();
					e.stopPropagation();
			...
			if (typeof(YT) === 'undefined' || typeof(YT.Player) === 'undefined') {	
				setTimeout( function() {
					if ( typeof window.onYouTubePlayerAPIReady !== 'undefined' ) {
						typeof window.gambitOtherYTAPIReady === 'undefined' ? window.gambitOtherYTAPIReady = [] : window.gambitOtherYTAPIReady.push( window.onYouTubePlayerAPIReady );
					}
					window.onYouTubePlayerAPIReady = function() {
						createplayer();
						if ( typeof window.gambitOtherYTAPIReady !== 'undefined' ) {
							if ( window.gambitOtherYTAPIReady.length ) window.gambitOtherYTAPIReady.pop()();
						}
					};
				}, 2);
			} else {
			...
			const playAndpause = function(e) {
				if (playerready) {
					let title;
					if (player.getPlayerState() === 1) {
						player.pauseVideo();
						title = 'Lire (k)';
						window.isplayerpaused = true;
						removeNotes("mediabox-content");
						stopAnime();
					} else {
						player.playVideo();
			...
			const volumeChange = function(val) {
				if (playerready) {
					const volumeactuel = player.getVolume();
					val = val * sautvolume;
					if ((val < 0 && volumeactuel === 0) || (val > 0 && volumeactuel === 100)) val = 0;
					const volume = volumeactuel + val;
			...
			const focusedControls = function(b){
			...
			const handleTimelineUpdate = function(e) {
				e.preventDefault();
				e.stopPropagation();
				...
				const currenttime = percent * player.getDuration();
			...
			const toggleScrubbing = function(e) {
				const rect = timelineContainer.getBoundingClientRect();
				const percent = Math.min(Math.max(0, e.x - rect.x), rect.width) / rect.width;
				const currenttime = percent * player.getDuration();
			...
			/*function changeOverlay(val){
				mediaboxbgoverlay.style.backgroundColor = "rgba(20, 20, 20," + val + ")";
			}*/
			...
			function skip(durer) {
				if (playerready){
					...
					const currenttime = player.getCurrentTime() + durer;
					const diff = parseFloat((currenttime/duration));
					player.seekTo(currenttime);
					timelineContainer.style.setProperty("--progress-position", diff);
					changeOverlay(diff);
			...
			function setVideoTime(num) {
				if (playerready){
					...
					num = num / 10;
					const duration = player.getDuration();
					const currenttime = duration * num;
					player.seekTo(currenttime);
			...
			function handleSingleClick(e) {
				...
				if(window.noMobile) {
					...
					toogleclick ? focusedControls(false) : mouseHideelement(e);
			...
			document.addEventListener("fullscreenchange",function() {
				...
				let title;
				const btnsetting = content.querySelector('.button-setting');
			...
			this.toggleMiniLecteure = function() {
				...
				if(window.isMinimised){
					...
					setTimeout(function(){
						this.sceneenglobe.style.setProperty('--rotate-sceneenglobe',`0deg`);
						this.sceneenglobe.style.setProperty('--scale',`4`);
					...
					this.sceneenglobe.style.cssText = this.stylescene;
			...
			const toggleMute = function() {	
				if (playerready){
					...
					if(player.isMuted()){
						...
						window.globalvolume >= 50 ? videoContainer.dataset.volumeLevel = "high" : videoContainer.dataset.volumeLevel = "low";
						title = 'Désactiver le son (s)';
						Notification("Volume: ", window.globalvolume + "%");
						window.globalmute = false;
						updateTitle(muteBtn,title);
						if(player.getPlayerState() === 1) addNotes("mediabox-content");
					}else{
						changeIconeMute();
						removeNotes("mediabox-content");
					}
			...
			function changeIconeMute(){
				...
				let title = 'Activer le son (s)';
				Notification("Volume: ", "0%");
				window.globalmute = true;
				updateTitle(muteBtn,title);
			}
			
			const toogleIconVolume = debounce(function() {
				const ytvolume = player.getVolume();
			...
			mediaboxclose.addEventListener('click', function(e) {
				e.stopPropagation()
				if(window.isMinimised){
					setTimeout(function(){
						this.root.classList.remove('minilecteur');
			...
			const toogleSetting = function(b) {
				let title;
			...
			content.addEventListener("touchend", function(e) {
				...
				betweenY > 0 ? orientationY = "dwn" : orientationY = "top";
		...
		initElement: function(el,str) {
			...
			triggerEvent(parent, 'mouseleave');
			...
			if(!window.isMinimised){
				this.stylescene = this.sceneenglobe.style.cssText;
				this.sceneenglobe.style.setProperty('--rotate-sceneenglobe',`0deg`);
				this.sceneenglobe.style.setProperty('--scale',`4`);
				if (this.video.played) {setTimeout(function(){this.video.pause()}.bind(this),500);}
			}
			const link = this.parseUrl(el.getAttribute('href'));
			const imagelink = el.dataset.img;
			const title = el.querySelector('.yttitle').textContent;
			this.currenttime = el.dataset.currenttime;
			const globalid = el.dataset.globalid;
			let globalidsplit = globalid.split(',');
			const lastpid = globalidsplit[0];
			const lastvid = globalidsplit[1];
		...
		close: function() {
			...
			let timer = null;
			if (timer) clearTimeout(timer);
			if (this.player) cell.dataset.currenttime = this.player.getCurrentTime();
